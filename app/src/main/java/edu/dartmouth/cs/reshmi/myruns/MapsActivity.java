...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onSaveClicked(View v) {
        ...
        if (mExerciseEntry != null) {
            ...
            //Check if inferred activities exist. If yes then do majority voting.
            if(mExerciseEntry.getmActivityCount().size() > 0) {
                ArrayList<String> activityCount = mExerciseEntry.getmActivityCount();
                int run = 0, walk = 0, stand = 0, unknown = 0;
                for (int i = 0; i < activityCount.size(); i++) {
                    if (activityCount.get(i).toString().equals("Running"))
                        run++;
                    else if (activityCount.get(i).toString().equals("Standing"))
                        stand++;
                    else if (activityCount.get(i).toString().equals("Walking"))
                        walk++;
                    else
                        unknown++;
                }
                String ans = "";
                if (run > stand && run > walk && run > unknown)
                    ans = "Running";
                else if (run < stand && stand > walk && stand > unknown)
                    ans = "Standing";
                else if (walk > stand && run < walk && walk > unknown)
                    ans = "Walking";
                else
                    ans = "Unknown";

                //update location
                mExerciseEntry.setmActivityType(ans);
            }
